...
router.post("/", upload.single("app_logo"), async (req, res) => {
  ...
  if (!req.file)
    return res.status(400).send("Please upload a jpeg or png file");
...
router.put("/:id", upload.single("app_logo"), async (req, res) => {
  ...
  if (!setting)
    return res
      .status(404)
      .send("The application setting with the given ID was not found.");
...
router.delete("/:id", async (req, res) => {
  ...
  if (!setting)
    return res
      .status(404)
      .send("The app setting with the given ID was not found.");
